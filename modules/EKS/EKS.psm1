...
function Get-EKSToken {
    param (
        ...
        $ProfileName = $( if (-Not $env:AWS_PROFILE) { return $env:AWS_PROFILE } else { return "default" } ),
        [Switch] $PassThru,
        [Switch] $Raw
    )

    $response = Invoke-Expression "aws eks get-token --cluster-name `"$Clustername`" --region `"$Region`" --profile `"$ProfileAWS`"" | ConvertFrom-Json
